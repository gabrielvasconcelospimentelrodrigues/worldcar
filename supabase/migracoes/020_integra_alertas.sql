-- Alertas da fidelidade entram na mesma sincronizacao dos demais.
--
-- Uma funcao que so roda quando alguem lembra de chama-la nao avisa nada. A
-- sincronizacao ja e disparada ao abrir o financeiro e o painel, entao e ali
-- que o aviso de ponto vencendo precisa estar.

-- A parte operacional (orcamento expirado, conta atrasada, entrega atrasada,
-- cobranca) sai para uma funcao propria, e `sincronizar_pendencias` passa a ser
-- o ponto unico que chama as duas.
CREATE OR REPLACE FUNCTION public.sincronizar_operacao()
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
...
revoke all on function public.sincronizar_operacao from public, anon;
grant execute on function public.sincronizar_operacao to authenticated;
...
begin
  ...
  v_base := public.sincronizar_operacao();
  v_fid  := public.alertas_fidelidade();

  return v_base || jsonb_build_object('alertas_fidelidade', v_fid->'vencendo');
end;
$$;
