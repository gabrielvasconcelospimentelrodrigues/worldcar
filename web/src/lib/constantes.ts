...
export const TIPO_FORNECEDOR: Record<TipoFornecedor, string> = {
  PECAS: "Peças",
  TINTAS: "Tintas",
  INSUMOS: "Insumos",
  PELICULAS: "Películas",
...
export const STATUS_COTACAO: Record<StatusCotacao, Rotulo> = {
  ABERTA: { label: "Aguardando resposta", cor: "bg-amber-600 text-white" },
  RESPONDIDA: { label: "Respondida", cor: "bg-blue-600 text-white" },
  DECIDIDA: { label: "Decidida", cor: "bg-emerald-600 text-white" },
  CANCELADA: { label: "Cancelada", cor: "bg-carvao-600 text-white" },
};

export const UNIDADES = ["un", "pç", "cx", "kg", "L", "m", "m²", "jogo", "par", "serviço"];

/**
 * Niveis do programa de fidelidade. Vive aqui, e nao na tela, porque aparece na
 * lista de clientes, na ficha e no painel do programa — e tres definicoes da
 * mesma cor acabam divergindo.
 */
export const NIVEL_FIDELIDADE: Record<string, { cor: string; rotulo: string; minimo: string }> = {
  DIAMANTE: { cor: "bg-sky-100 text-sky-800",       rotulo: "Diamante", minimo: "R$ 15.000" },
  OURO:     { cor: "bg-amber-100 text-amber-800",   rotulo: "Ouro",     minimo: "R$ 5.000" },
  PRATA:    { cor: "bg-carvao-200 text-carvao-700", rotulo: "Prata",    minimo: "R$ 1.500" },
  BRONZE:   { cor: "bg-orange-100 text-orange-800", rotulo: "Bronze",   minimo: "abaixo disso" },
};
