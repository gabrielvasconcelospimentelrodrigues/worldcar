...
/**
 * Campo com mascara de digitacao.
 *
 * O input visivel mostra o valor pontuado; um input oculto com o `name` carrega
 * o valor limpo. Assim os formularios que leem `new FormData(form)` continuam
 * funcionando sem alteracao, e o banco nunca recebe pontuacao.
 *
 *   <CampoMascara rotulo="Telefone" name="telefone" mascara={mascararTelefone}
 *                 limpar={soDigitos} />
 */
export function CampoMascara({
  ...
  /** Como gravar: sem isso, grava o texto mascarado. */
  limpar?: (v: string) => string;
  aoMudar?: (mascarado: string, cru: string) => void;
}) {
  const [texto, setTexto] = useState(() => mascara(String(defaultValue ?? "")));
  const cru = limpar ? limpar(texto) : texto;
  ...
  return (
    <div className={className}>
      ...
      <input
        ...
        value={texto}
        onChange={(e) => {
          const m = mascara(e.target.value);
          setTexto(m);
          aoMudar?.(m, limpar ? limpar(m) : m);
        }}
